...
str(NEW)
write.csv(NEW, file = "C:\\Users\\TrevorWhite\\Downloads\\2016Baseball.csv")
...
empty_df <- data.frame(matrix(nrow = 300, ncol = 2))
colnames(NEW)->colnamesb
colnames(NEW2016)->cals
empty_df$cals <- cals
data.frame(colnamesb,cals)->colnamesb
# Create an empty dataframe with 2 columns
colnames_df <- data.frame(matrix(nrow = 300, ncol = 23))
colnames(colnames_df) <- c("df1_colnames", "df2_colnames")

# Go through the column names of each dataframe and add them to the new dataframe
for (i in 1:length(colnames(baseball))) {
  colnames_df[i,1] <- colnames(baseball)[i]
}
for (i in 1:length(colnames(NEW))) {
  colnames_df[i,2] <- colnames(NEW)[i]
}
rbind(baseball,NEW2016,NEW)->mastbase
names(baseball)->basenames
names(NEW)<-basenames
mastbase->baseball
